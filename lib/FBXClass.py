...
class FBXClass(object):
	...
	def save(self, filename = None, pFileFormat=0):
		"""
		Save the current fbx scene as the incoming filename .fbx
		"""
		# save as a different filename
		if not filename is None:
			FbxCommon.SaveScene(self.sdk_manager, self.scene, filename, pFileFormat=pFileFormat)
		else:
			FbxCommon.SaveScene(self.sdk_manager, self.scene, self.filename, pFileFormat=pFileFormat)
		self.close()